<?php
    ...
    class ModelPayment extends DAO
    {
        ...
        public function install() {
            ...
            $description[osc_language()]['s_title'] = '{WEB_TITLE} - Opção para publicar o seu anúncio: {ITEM_TITLE}';
            $description[osc_language()]['s_text'] = '<p>Hi {CONTACT_NAME}!</p><p>Nós apenas publicamos seu artigo ({ITEM_TITLE}) em {WEB_TITLE}.</p><p>{START_PUBLISH_FEE}</p><p>A fim de tornar o seu anúncio disponível para qualquer pessoa na {WEB_TITLE}, você deve concluir o processo e pagar a taxa de publicação. Você poderia fazer isso no seguinte link: {PUBLISH_LINK}</p><p>{END_PUBLISH_FEE}</p><p>{START_PREMIUM_FEE}</p><p>Você poderia fazer o Destaque de anúncio e torná-lo visivel em primeiro resultado das pesquisas realizadas em{WEB_TITLE}. Você poderia fazer isso no seguinte link: {PREMIUM_LINK}</p><p>{END_PREMIUM_FEE}</p><p>este é um e-mail automático, se você já fez isso, por favor, ignore este e-mail.</p><p>Obrigado!</p>';
