...
<html xmlns="http://www.w3.org/1999/xhtml" dir="ltr" lang="<?php echo str_replace('_', '-', osc_current_user_locale()); ?>">
    ...
    <body class="psss">
        ...
        <div class="content add_item">
            <h1><strong><?php _e('Publish a listing', 'one'); ?></strong></h1>
            <ul id="error_list"></ul>			
            <form name="item" action="<?php echo osc_base_url(true);?>" method="post" enctype="multipart/form-data">
                <fieldset>
                    ...
                    <div class="box general_info">
                        ...
                        <div class="row dess">
							<label><?php _e('Descrição', 'one'); ?><span>*</span></label>
                    ...
                    <?php if( osc_price_enabled_at_items() ) { ?>
						<div class="box price">
							<label for="price"><?php _e('Price', 'one'); ?></label>
							<?php ItemForm::price_input_text(); ?>
							<?php ItemForm::currency_select(); ?>
						</div><font color= "#417cd4"><strong><center>( Ex: Mil reais - Colque assim: 1.000 ) Insira somente números inteiros sem a vígula.</center></strong></font>
					<?php } ?>
                    ...
                    <div class="box location">
                        <div class="row">
                            <label for="countryId"><?php _e('Country', 'one'); ?></label>
                            <?php ItemForm::country_select(osc_get_countries(), osc_user()); ?>
						</div>
                        <div class="row">
                            <label for="regionId"><?php _e('Region', 'one'); ?></label>
					...
					<div class="box seller_info">
						<div class="row">
                            <label for="city"><?php _e('Telefone', 'one'); ?></label>
                            <?php ItemForm::city_area_text(osc_user()); ?>							
						</div>
						<?php if(!osc_is_web_user_logged_in() ) { ?>                    
							<div class="row">
								<label for="contactName"><?php _e('Name', 'one'); ?></label>
								<?php ItemForm::contact_name_text(); ?>
							</div>
							<div class="row">
								<label for="contactEmail"><?php _e('E-mail', 'one'); ?><span>*</span></label>
								<?php ItemForm::contact_email_text(); ?>
							</div>
							<div class="row">
								<div class="m_sheck" style="width: 230px;text-align: right;float:left;">
									<?php ItemForm::show_email_checkbox(); ?>
								</div>
								<label for="showEmail" style="width: 250px !important;text-align:left;"><?php _e('Show e-mail on the listing page', 'one'); ?></label>
							</div>
						<?php }; ?>
					</div>
					<div class="plugin_zone">
					...
					<div class="btpb">
						<div class="test"></div>
						<button type="submit"><?php _e('Publish', 'one'); ?></button>
					</div>
				</fieldset>
			</form>
		</div>	
